...
Dataset loader for ARANet-B denoising training and evaluation.
Loads clean images and synthesises noisy/clean pairs on the fly
with a noise-level map concatenated as an extra channel.
"""
import random
import numpy as np
import torch
...
class DatasetARANetB(data.Dataset):
    """
    # -----------------------------------------
    # Get L/H/M for denosing on AWGN with a range of sigma.
    # Only dataroot_H is needed.
    # -----------------------------------------
    # e.g., ARANet-B input: H = f(cat(L, M)), where M is the noise level map
    # -----------------------------------------
    """

    def __init__(self, opt):
        super(DatasetARANetB, self).__init__()
        self.opt = opt
